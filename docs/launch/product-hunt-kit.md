...
Five branded cards are generated by `scripts/make_gallery_cards.py` into `docs/launch/assets/gallery/`, at PH's 1270x760:

| File | Carries |
|---|---|
| `01-hero.png` | Positioning, plus the MIT / BYOK / no-server rail |
| `02-live-copilot.png` | The five live agents, one line each |
| `03-citations.png` | The differentiator: every number traces to a source |
| `04-council.png` | The four-agent pitch pipeline |
| `05-open-source.png` | Self-hosted, your keys, and an honest note on today's limits |

They are deliberately typographic and diagrammatic rather than imitation UI. Fake product screenshots read as mockups the moment anyone looks closely, and cost more trust than they buy.
